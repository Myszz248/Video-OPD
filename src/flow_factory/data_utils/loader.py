...
def get_dataloader(
    ...
    **kwargs,
    ...
    column_hash_strs = [
        f"{name}={getattr(data_args, name, None)}"
        for name in (
            "prompt_column",
    ...
    base_kwargs = {
        ...
        "extra_hash_strs": [
            GeneralDataset.OUTPUT_SCHEMA_VERSION,
            config.model_args.model_type,
            config.model_args.model_name_or_path,
            *column_hash_strs,
            f"first_frame_column={getattr(data_args, 'first_frame_column', None)}",
            f"first_frame_ar_tol={getattr(data_args, 'first_frame_aspect_ratio_tolerance', None)}",
        ],
    }
    base_kwargs.update(filter_kwargs(GeneralDataset.__init__, **data_args))
    base_kwargs["force_reprocess"] = data_args.force_reprocess

    # First-frame aspect-ratio filtering compares against the per-split generation
    # resolution, so it must be threaded in per split (and folded into the cache key).
    enable_first_frame_filter = getattr(data_args, "first_frame_column", None) is not None

    def _split_base_kwargs(split_base_kwargs: dict, resolution) -> dict:
        if not enable_first_frame_filter:
            return split_base_kwargs
        target_resolution = tuple(resolution)
        return {
            **split_base_kwargs,
            "target_resolution": target_resolution,
            "extra_hash_strs": [
                *split_base_kwargs["extra_hash_strs"],
                f"first_frame_target_resolution={target_resolution}",
            ],
        }
    ...
    dataset = _create_or_load_dataset(
        split="train",
        accelerator=accelerator,
        base_kwargs=_split_base_kwargs(
            {**base_kwargs, "preprocess_kwargs": train_preprocess_kwargs},
            training_args.resolution,
        ),
    ...
    if GeneralDataset.check_exists(data_args.dataset, "test"):
        ...
        test_dataset = _create_or_load_dataset(
            ...
            accelerator=accelerator,
            base_kwargs=_split_base_kwargs(
                {**base_kwargs, "preprocess_kwargs": test_preprocess_kwargs},
                eval_args.resolution,
            ),
            enable_distributed=enable_distributed,
            preprocess_parallelism=preprocess_parallelism,
        )
