        ...
        **kwargs,
        ...
        for comp_name, target_modules in self.target_module_map.items():
            ...
            if not target_modules:
                continue
            save_targets.append((comp_name, target_modules))

        if not save_targets:
            raise ValueError(
                "Refusing to create an empty checkpoint because no saveable target components "
                "were found. "
                f"finetune_type={self.model_args.finetune_type!r}, "
                f"target_components={self.model_args.target_components!r}, "
                f"target_module_map={self.target_module_map!r}."
            )
        
        with save_context():
            for comp_name, target_modules in save_targets:
                component = self.get_component(comp_name)
