...
class OPDTrainingArguments(TrainingArguments):
    ...
    opd_loss_weighting: Literal['flow_opd', 'uniform'] = field(
        default='flow_opd',
        metadata={
            "help": (
                "Per-timestep OPD loss weighting. `flow_opd` applies the time-weighted "
                "velocity MSE derived by Flow-OPD; `uniform` keeps legacy unweighted MSE."
            )
        },
    )
    ...
    num_train_timesteps: int = field(
        ...
        metadata={
            "help": (
                "Number of OPD training timesteps. In trajectory mode, this selects stored "
                "student denoising steps when `opd_trajectory_indices` is unset. 0 defaults to "
    ...
    time_sampling_strategy: Literal[
        ...
        metadata={
            "help": (
                "Time sampling strategy for OPD. Trajectory mode supports discrete, "
                "discrete_with_init, and discrete_wo_init."
            )
        },
    )
    time_shift: float = field(
    ...
    opd_timestep_mode: Literal['trajectory', 'forward_process'] = field(
        default='trajectory',
        metadata={
            "help": (
                "OPD timestep source. `trajectory` supervises stored student denoising states; "
                "`forward_process` keeps the legacy final-latent noising objective."
            )
        },
    )
    ...
    store_student_trajectory: bool = field(
        default=True,
        metadata={
            "help": "Must be true for trajectory-mode OPD so rollout latents are stored."
        },
    )
    opd_trajectory_indices: Optional[List[int]] = field(
        ...
        metadata={
            "help": (
                "Optional denoising step indices for trajectory-mode OPD. Negative values "
                "follow Python indexing over denoising steps; -1 is the final denoising step "
                "and is invalid with `opd_loss_weighting='flow_opd'`. None derives a sparse "
                "set from num_train_timesteps."
            )
        },
    )
    opd_blend_teacher_student_latents: bool = field(
        default=False,
        metadata={
            "help": (
    ...
    opd_teacher_blend_weight: float = field(
        default=0.5,
        metadata={
            "help": (
                "Teacher-first-step weight used when "
                "`opd_blend_teacher_student_latents=true`. No automatic normalization is applied."
            )
        },
    )
    opd_student_blend_weight: float = field(
        default=0.5,
        metadata={
            "help": (
                "Student-trajectory weight used when "
                "`opd_blend_teacher_student_latents=true`. No automatic normalization is applied."
            )
        },
    )
    def __post_init__(self):
        ...
        if self.opd_teacher_blend_weight < 0:
            raise ValueError(
                "`opd_teacher_blend_weight` must be non-negative, got "
                f"{self.opd_teacher_blend_weight}."
            )
        if self.opd_student_blend_weight < 0:
            raise ValueError(
                "`opd_student_blend_weight` must be non-negative, got "
                f"{self.opd_student_blend_weight}."
            )
        if (
            self.opd_blend_teacher_student_latents
            and self.opd_teacher_blend_weight == 0
            and self.opd_student_blend_weight == 0
        ):
            raise ValueError(
                "`opd_blend_teacher_student_latents=true` requires at least one positive latent "
                "blend weight."
            )
        if self.opd_loss_weighting == 'flow_opd' and self.opd_loss_type != 'velocity_mse':
            raise ValueError(
                "`opd_loss_weighting='flow_opd'` is derived for `opd_loss_type='velocity_mse'`. "
                "Use `opd_loss_weighting='uniform'` for legacy x0-space supervision."
            )
        ...
        if self.kl_type not in ['v-based']:
            raise ValueError(f"Invalid KL type: {self.kl_type}. Valid options are: ['v-based'].")
        if self.opd_timestep_mode not in ['trajectory', 'forward_process']:
            raise ValueError(
                "Invalid OPD timestep mode: "
                f"{self.opd_timestep_mode!r}. Valid options are: "
                "['trajectory', 'forward_process']."
            )
        if self.opd_timestep_mode == 'trajectory':
            if not self.store_student_trajectory:
                raise ValueError(
                    "`store_student_trajectory` must be true when "
                    "`opd_timestep_mode='trajectory'`."
                )
            if self.time_sampling_strategy not in [
                'discrete',
                'discrete_with_init',
                'discrete_wo_init',
            ]:
                raise ValueError(
                    "`opd_timestep_mode='trajectory'` supports only discrete timestep "
                    f"selection, got {self.time_sampling_strategy!r}."
                )
            if self.opd_trajectory_indices is not None:
                if not self.opd_trajectory_indices:
                    raise ValueError("`opd_trajectory_indices` must not be empty.")
                self.num_train_timesteps = len(self.opd_trajectory_indices)
            if self.opd_blend_teacher_student_latents and self.num_inference_steps < 2:
                raise ValueError(
                    "`opd_blend_teacher_student_latents=true` requires `num_inference_steps >= 2` "
                    "so the teacher can produce a first denoising-step latent."
                )
        elif self.opd_trajectory_indices is not None:
            raise ValueError(
                "`opd_trajectory_indices` is only used with `opd_timestep_mode='trajectory'`. "
                "Set it to null for the legacy forward_process mode."
            )
        if self.opd_blend_teacher_student_latents and self.opd_timestep_mode != 'trajectory':
            raise ValueError(
