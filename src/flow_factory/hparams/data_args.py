...
class DataArguments(ArgABC):
    ...
    first_frame_column: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "Raw dataset column holding the teacher first-frame image path (e.g. "
                "'first_frame_path'). When set, samples whose first-frame aspect ratio "
                "deviates from the target video resolution by more than "
                "`first_frame_aspect_ratio_tolerance` are dropped during preprocessing, so the "
                "teacher's first-frame condition is never badly stretched. The column is only "
                "read for filtering; it is left untouched so downstream teacher context still "
                "sees it. Leave None to disable aspect-ratio filtering."
            )
        },
    )
    first_frame_aspect_ratio_tolerance: float = field(
        default=0.2,
        metadata={
            "help": (
                "Maximum allowed relative aspect-ratio deviation between the first-frame image "
                "and the target video resolution, measured as "
                "max(ar_image / ar_video, ar_video / ar_image) - 1. For example, 0.2 keeps only "
                "samples whose first-frame aspect ratio is within +/-20% of the video aspect "
                "ratio. Only used when `first_frame_column` is set."
            )
        },
    )
