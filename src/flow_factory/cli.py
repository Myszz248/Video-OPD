...
def _get_external_world_size() -> int:
    """Return WORLD_SIZE from an external launcher, defaulting to single-process."""
    raw_world_size = os.environ.get("WORLD_SIZE")
    if raw_world_size is None or raw_world_size == "":
        return 1
    try:
        return int(raw_world_size)
    except ValueError:
        logger.warning(
            "Ignoring invalid WORLD_SIZE=%r while deciding whether an external "
            "distributed launcher is active.",
            raw_world_size,
        )
        return 1


def _clean_env_for_accelerate_launch() -> dict[str, str]:
    """Remove stale distributed worker variables before spawning accelerate."""
    child_env = os.environ.copy()
    removed = []
    for key in _DISTRIBUTED_LAUNCH_ENV_VARS:
        if key in child_env:
            removed.append(f"{key}={child_env.pop(key)}")
    if removed:
        logger.warning(
            "Removed stale distributed environment variables before accelerate launch: %s",
            ", ".join(removed),
        )
    ...
    if src_dir not in pythonpath_entries:
        child_env["PYTHONPATH"] = os.pathsep.join([src_dir, *pythonpath_entries])
        logger.info("Prepended current source directory to child PYTHONPATH: %s", src_dir)
    return child_env
...
def train_cli():
    ...
    logger.info("Flow-Factory CLI module: %s", __file__)
    ...
    external_world_size = _get_external_world_size()
    child_env = None

    if os.environ.get("RANK") is not None and external_world_size <= 1 and num_procs > 1:
        logger.warning(
            "Detected RANK=%s but WORLD_SIZE=%s, so this does not look like an active "
            "multi-process launcher. Continuing with accelerate launch for num_processes=%s.",
            os.environ.get("RANK"),
            os.environ.get("WORLD_SIZE", "unset"),
            num_procs,
        )

    if external_world_size > 1 or num_procs <= 1:
        ...
        child_env = _clean_env_for_accelerate_launch()
        cmd = [
