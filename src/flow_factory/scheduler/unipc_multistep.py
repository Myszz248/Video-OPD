...
class UniPCMultistepSDEScheduler(UniPCMultistepScheduler, SDESchedulerMixin):
    ...
    def step(
        ...
        # 0. Evaluation delegates the actual denoising update to the parent
        #    UniPC multistep predictor-corrector. That update relies on the
        #    inference schedule populated by `set_timesteps`, so only invoke it
        #    when the caller actually requests stepped latents. Callers that only
        #    need the raw model prediction (e.g. OPD step-level velocity targets
        #    request `noise_pred` without `next_latents`) must still receive a
        #    populated `noise_pred` and must not trigger `super().step()`.
        if self.is_eval:
            needs_step = any(key in return_kwargs for key in ("next_latents", "next_latents_mean"))
            eval_output: Dict[str, Any] = {}
            if "noise_pred" in return_kwargs:
                eval_output["noise_pred"] = noise_pred.float()
            if needs_step:
                step_output = super().step(noise_pred, timestep, latents, return_dict=False)
                next_latents_mean = step_output[0]
                eval_output["next_latents"] = next_latents_mean
                eval_output["next_latents_mean"] = next_latents_mean
            if not return_dict:
                return (
                    eval_output.get("next_latents"),
                    None,
                    eval_output.get("next_latents_mean"),
                    None,
                    None,
                )
            return UniPCMultistepSDESchedulerOutput.from_dict(eval_output)
