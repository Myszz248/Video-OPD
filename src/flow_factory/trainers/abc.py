...
class BaseTrainer(ABC):
    ...
    def _ensure_deepspeed_micro_batch_size(self) -> None:
        """Set DeepSpeed micro batch size when dataloaders bypass accelerator.prepare()."""
        ...
        configured = deepspeed_config.get("train_micro_batch_size_per_gpu")
        if configured in (None, "auto"):
            deepspeed_config["train_micro_batch_size_per_gpu"] = micro_batch_size
            logger.info(
                "Set DeepSpeed train_micro_batch_size_per_gpu(%d) from "
                "per_device_batch_size(%d) because the training dataloader is not passed "
        ...
        if int(configured) != micro_batch_size:
            logger.warning(
                "DeepSpeed train_micro_batch_size_per_gpu(%s) differs from "
                "per_device_batch_size(%d). Keeping the explicit DeepSpeed value.",
                configured,
                self.training_args.per_device_batch_size,
            )
    ...
    def _initialization(self):
        ...
        self._ensure_deepspeed_micro_batch_size()
