...
class OPDContextBuilder:
    ...
    def __init__(
        ...
        text_context_keys = [key for key in self.context_keys if key not in MEDIA_CONTEXT_KEYS]
        if text_context_keys and "{context}" not in self.prompt_template:
            raise ValueError(
                "`teacher.prompt_template` must contain `{context}` when "
                "`train.teacher_context_keys` contains text context keys. "
                f"Text context keys: {text_context_keys}. Media-only context keys "
                "can use a prompt template without `{context}`."
            )
