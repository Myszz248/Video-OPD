...
- **Decoupled paradigm (DPO, DiffusionNFT, AWM, DGPO, CRD)**: Training timesteps are decoupled from the actual sampling dynamics, making them inherently solver-agnostic — any ODE solver can be used for trajectory generation without modifying the training procedure.
- **Trajectory-distillation paradigm (OPD)**: Teacher supervision is applied to latent states recorded from the student's own denoising trajectory. It does not require log-probability estimates; the Flow-OPD-faithful default uses Flow-SDE rollout with time-weighted velocity matching, while legacy ODE rollout remains available only with `opd_loss_weighting: "uniform"`.
...
> **Note**: `ODE` dynamics produce deterministic trajectories and cannot provide log-probability estimates. Therefore, `ODE` can only be used with decoupled algorithms such as `NFT`, `AWM`, and `DGPO`. For OPD, `ODE` is only valid for the explicit legacy `opd_loss_weighting: "uniform"` path; the default Flow-OPD objective requires `Flow-SDE`. See the [DiffusionNFT](#diffusionnft), [AWM](#awm-advantage-weighted-matching), [DGPO](#dgpo), and [OPD](#opd-step-level-teacher-distillation) sections.
...
> **Note**: DPO, DGPO, DiffusionNFT, AWM, and CRD are forward-diffusion based RL algorithms, which decouple training from sampling dynamics and are solver-agnostic — any ODE/SDE solver can be used for trajectory generation.
...
`opd` implements the step-level migration of OPD-style supervision from token-level language generation to flow-matching video generation. The student rolls out videos from text only; the frozen teacher is evaluated on stored latent states from the student's own denoising trajectory. By default, the teacher and student use the same `prompt`, which makes prompt-only datasets such as `dataset/vid_prompt` usable for the first method-validation run. Teacher-only context from dataset metadata, such as `dense_caption`, `scene_graph`, or `first_frame_path`, can be enabled by setting `teacher_context_keys`.

The default trajectory objective is:

```text
student rollout: x_i -> x_{i+1}
loss = w(t) * ||v_student(x_t, t, prompt) - stopgrad(v_teacher(x_t, t, prompt))||^2
```

The default `opd_loss_weighting: "flow_opd"` uses the time-weighted velocity MSE
derived by Flow-OPD [[14]](#ref14), so the dense teacher signal matches the
continuous reverse-KL objective under Flow-SDE rollout. Set
`opd_loss_weighting: "uniform"` only for the legacy unweighted ODE-style
objective.
For this default objective, leave both `scheduler.sde_steps` and
`scheduler.num_sde_steps` as `null` so every non-final denoising transition has
positive Flow-SDE variance. Custom sparse-SDE rollouts should use the explicit
legacy `uniform` objective unless their KL weighting is re-derived.

When optional teacher-only context is enabled, the teacher side becomes
`v_teacher(x_t, t, prompt + context)` for text metadata, or
`v_teacher(x_t, t, prompt, media_context)` for image/video-capable teachers such as `wan2_i2v` and `wan2_vace`. The student remains conditioned only on `prompt`.

Unlike NFT/AWM-style forward-process training, trajectory-mode OPD does not reconstruct `x_t` by adding fresh noise to the final latent. It stores a sparse set of rollout latents and replays those same student states during optimization. Set `opd_trajectory_indices` to choose exact denoising steps, or leave it `null` to derive an evenly spaced sparse set from `num_train_timesteps`, `time_sampling_strategy`, and `timestep_range`. With the default `opd_loss_weighting: "flow_opd"`, automatic selection excludes the final denoising step because the default Flow-SDE scheduler has no positive transition variance there; explicit indices must also be non-final.

When `opd_blend_teacher_student_latents: true`, OPD adds one extra teacher-only sampling action during `sample()`: the frozen teacher rolls out exactly one denoising step from the student's exact initial rollout noise while using teacher-only prompt/context conditioning, then stores that first-step latent. During `optimize()`, each selected student trajectory latent is blended with the stored teacher first-step latent using `opd_teacher_blend_weight` and `opd_student_blend_weight`, and the teacher target is evaluated on that blended latent instead of the raw student state. This path is intended for first-frame-conditioned teachers such as `wan2_vace`.
...
train:
  ...
  opd_trajectory_indices: null     # Optional; Flow-OPD explicit indices must be non-final
  opd_blend_teacher_student_latents: false  # Options: true, false
...
If teacher-only context is enabled, dataset rows can keep that information under `opd_context` so it never enters the student adapter:
...
For CSV datasets, set `data.extra_kwargs.prompt_column` to choose the student prompt column while keeping all other columns available as teacher-only metadata. `first_frame` / `first_frame_path` are supported for `wan2_i2v` and `wan2_vace` teachers; unsupported media keys such as `last_frame`, `depth`, `pose`, `optical_flow`, and `object_tracks` still fail fast until an adapter-specific encoder is added. For a runnable prompt-only Wan2.1 setup, see `examples/opd/lora/wan21/t2v_reward_teacher.yaml`; for first-frame contextual VACE distillation, see `examples/opd/lora/wan2_vace/pexels_first_frame_context.yaml`.
...
* <a name="ref14"></a>[14] [**Flow-OPD**: On-Policy Distillation for Flow Matching Models](https://arxiv.org/abs/2605.08063)
