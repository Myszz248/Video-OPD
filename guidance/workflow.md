...
| **OPD** | `False` | Sparse trajectory steps | Time-weighted teacher velocity matching on stored student denoising states; teacher uses the same prompt by default, with optional teacher-only `opd_context` |
...
- **SDE dynamics for exploration**: GRPO injects noise during sampling via SDE formulation, enabling the log-probability computation required for policy gradients. NFT, AWM, DGPO, and CRD use decoupled sampling with `compute_log_prob=False`. OPD also uses `compute_log_prob=False`; the Flow-OPD-faithful path uses Flow-SDE rollout states but optimizes the closed-form, time-weighted velocity MSE instead of a log-prob policy ratio.
...
| **OPD** | Replays stored student trajectory latents; applies Flow-OPD time-weighted velocity matching to a frozen teacher that receives teacher-only context |
...
- **Per-timestep iteration**: GRPO iterates over stored trajectory timesteps for policy ratios. NFT, AWM, DGPO, and CRD sample fresh timesteps independently of the sampling trajectory. OPD replays stored student trajectory latents for time-weighted teacher velocity matching.
