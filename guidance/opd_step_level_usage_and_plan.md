...
- Training objective: Flow-OPD time-weighted teacher velocity matching, i.e. minimize `w(t) * ||v_student(x_t,t,text) - stopgrad(v_teacher(x_t,t,text,context))||^2` on the same stored student rollout state `x_t, t`, with optional reward weighting and KL regularization.
...
- 字段建议：
  ...
  - `opd_loss_weighting: "flow_opd"`，use `"uniform"` only for the legacy unweighted objective
...
2. Add `OPDTrainer` with trajectory-mode teacher velocity matching on stored student denoising states.
